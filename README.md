...
How does pore-scale geometry act as a physical filter on active transport: quantifying the confinement-driven transition from run-and-tumble to trapped motion in single ABPs and topology-dependent trapping in active polymer chains and rings across silt-to-sand pore regimes.
...
Single Active Brownian Particles (ABPs) — how confinement shifts run-and-tumble dynamics to trapped motion, modulating effective diffusivity and trapping probability as a function of pore size and Péclet number.
Active polymer chains and rings — how topology (linear vs. ring) and stiffness (flexible vs. semiflexible) interact with pore confinement to determine conformational swelling/shrinking and COM diffusion.
...
```
where:
`v₀ = Pe × kBT/(γσ)` is the self-propulsion speed (Pe = Péclet number)
`ê(θ)` is the unit orientation vector
`D_R` is the rotational diffusion coefficient; persistence time `τ_R = 1/D_R`
`f_obs` is the WCA repulsion from obstacles
`η, ξ` is the Gaussian white noise
Key dimensionless number: Pe = Fa·σ/(kBT) is the ratio of active to thermal forces.
...
D_eff increases monotonically with L. In silt-like confinement, D_eff drops by roughly an order of magnitude relative to the bulk value. At large Pe, D_eff is enhanced but the suppression in tight pores persists, so we can say confinement dominates over activity at small L.
...
```
So D_eff is a monotonically decreasing function of P_trapped and the correspondence in Fig. 4 is a direct consequence of this relation.
...
Statistical mechanics of active matter — connects to Dr. Rumi De's lab (predator-prey, cooperative dynamics)
Defect dynamics in active media — the trapping/escaping phenomenology of semiflexible rings parallels defect-mediated dynamics in active nematics (upcoming IISER Pune project)
Active transport in porous environments — directly relevant to planned IIT Bombay project with Prof. Rajarshi Chakrabarti
Soft condensed matter — Rg scaling, polymer conformation analysis align with planned 4th-year project with Prof. Arindam Kundagrami
---
References
A. Bhattacharyay, "Directed transport in equilibrium," arXiv:1008.4992 (2011)
L. Theeyancheri, S. Chaki, T. Bhattacharjee, R. Chakrabarti, "Active dynamics of linear chains and rings in porous media," J. Chem. Phys. 159, 014902 (2023)
A. Al Harraq, G. Choi, S. S. Datta, J. W. Shaevitz, "Soil texture regulates bacterial motility and chemotactic recruitment to plant roots," PRX Life 4, 013034 (2026)
...
K. Kremer and G. S. Grest, "Dynamics of entangled linear polymer melts," J. Chem. Phys. 92, 5057 (1990)
